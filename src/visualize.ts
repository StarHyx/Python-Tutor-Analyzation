...
require("jquery");
require("../node_modules/qtip/css/qtip.css");
require("../node_modules/qtip/qtip.js");
...
export class OptFrontendWithTestcases extends OptFrontendSharedSessions {
	...
	runTestCase(id, codeToExec, firstTestLine) {
		...
		this.executeCodeAndRunCallback(
			codeToExec,
	...
	handleUncaughtException(trace) {
		...
		if (killerException) {
			var excObj = {
				killerException: killerException,
	...
	experimentalPopUpSyntaxErrorSurvey() {
		if (this.prevExecutionExceptionObjLst.length > 0) {
			...
			var prevExecutionExceptionObj = this.prevExecutionExceptionObjLst[
				this.prevExecutionExceptionObjLst.length - 1
			];
			...
			var bub = new SyntaxErrorSurveyBubble(
				this.myVisualizer,
				"pyCodeOutputDiv"
			);
      ...
      */
			...
			var surveyBubbleHTML =
				'<div id="syntaxErrBubbleContents">\
                                <div id="syntaxErrHeader">You just fixed the following error:</div>\
                                <div id="syntaxErrCodeDisplay"></div>\
                                <div id="syntaxErrMsg"></div>\
                                <div id="syntaxErrQuestion">\
			...
			$(bub.qTipContentID() + " #syntaxErrSubmitBtn").click(() => {
				...
				$.get("syntax_err_survey.py", { arg: JSON.stringify(resObj) }, function(
					dat
				) {});
			...
			bubbleAceEditor.setValue(
				prevExecutionExceptionObj.myAppState.code.rtrim(),
				-1
			);
			...
			$("#syntaxErrMsg").html(
				htmlspecialchars(
					prevExecutionExceptionObj.killerException.exception_msg
				)
			);

			// unbind scroll handler first, then bind new one
			this.myVisualizer.domRoot
			...
			$.get(
				"syntax_err_survey.py",
				{ arg: JSON.stringify(impressionObj) },
				function(dat) {}
			);
		}
	}
	...
	popupRuntimeErrorSurvey() {
		...
		if (this.prevExecutionRuntimeErrorMsg && noErrorsInCurTrace) {
			...
			var savedPrevExecutionRuntimeErrorLine = this
				.prevExecutionRuntimeErrorLine;
			var savedPrevExecutionRuntimeErrorCode = this
				.prevExecutionRuntimeErrorCode;
			...
			$(bub.qTipContentID() + " #syntaxErrSubmitBtn").click(() => {
				...
				$.get(
					"runtime_err_survey.py",
					{ arg: JSON.stringify(resObj) },
					function(dat) {}
				);
			...
			bub.redrawCodelineBubble(); // do an initial redraw to align everything
			(bubbleAceEditor as any).scrollToLine(
				savedPrevExecutionRuntimeErrorLine - 1,
				true
			);

			// don't forget htmlspecialchars
			$("#syntaxErrMsg").html(
				htmlspecialchars(savedPrevExecutionRuntimeErrorMsg)
			);

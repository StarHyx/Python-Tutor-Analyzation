  ...
  something with the strict format [false], [true] or [true, ...]. */
	...
	render() {
		...
		if (this.params.embeddedMode) {
			...
			base.append(
				'<div style="font-size: 8pt; margin-bottom: 10px;"><a href="http://pythontutor.com" target="_blank" style="color: #3D58A2;">Python Tutor</a> by <a href="https://twitter.com/pgbovine" target="_blank" style="color: #3D58A2;">Philip Guo</a>. Support with a <a href="http://pgbovine.net/support.htm" target="_blank">small donation</a>.</div>'
			);
			base.find("#codeFooterDocs").hide(); // cut out extraneous docs
		} else {
			// also display credits:
			base.append(
				'<div style="font-size: 9pt; margin-top: 5px; margin-bottom: 10px;">Created by <a href="https://twitter.com/pgbovine" target="_blank">@pgbovine</a>. Support with a <a href="http://pgbovine.net/support.htm" target="_blank">small donation</a>.</div>'
			);
		}
	...
	findPrevBreakpoint() {
		...
		if (this.sortedBreakpointsList.length == 0) {
			...
			// final edge case:
			var lastElt = this.sortedBreakpointsList[
				this.sortedBreakpointsList.length - 1
			];
			...
			if (lang === "js") {
				...
				if (this.owner.params.embeddedMode) {
					...
					this.domRoot
						.find("#langDisplayDiv")
						.html(
							'C (gcc 4.8, C11)<br/><font color="#e93f34">EXPERIMENTAL!</font> <a href="https://github.com/pgbovine/OnlinePythonTutor/blob/master/unsupported-features.md" target="_blank">known bugs/limitations</a>'
						);
				}
			} else if (lang === "cpp") {
				if (this.owner.params.embeddedMode) {
					...
					this.domRoot
						.find("#langDisplayDiv")
						.html(
							'C++ (gcc 4.8, C++11)<br/><font color="#e93f34">EXPERIMENTAL!</font> <a href="https://github.com/pgbovine/OnlinePythonTutor/blob/master/unsupported-features.md" target="_blank">known bugs/limitations</a>'
						);
				}
			} else {
			...
			this.domRoot
				...
				.html(
					htmlspecialchars(msg) +
						`
      <span style="font-size: 9pt; color: #666">(see <a href="https://github.com/pgbovine/OnlinePythonTutor/blob/master/unsupported-features.md" target="_blank">unsupported features</a>)</span>`
				)
				.show();
		} else {

...
export class OptLiveFrontend extends OptFrontendSharedSessions {
	...
	constructor(params) {
		...
		$("#legendDiv")
			...
			.append(
				'<p style="margin-top: 4px"><svg id="curLegendArrowSVG"/> 即将执行</p>'
			);
		...
		$("#jmpFirstInstr").click(() => {
			if (this.myVisualizer) {
				this.myVisualizer.renderStep(0);
			}
		});

		$("#jmpLastInstr").click(() => {
			if (this.myVisualizer) {
				this.myVisualizer.renderStep(this.myVisualizer.curTrace.length - 1);
			}
		});

		$("#jmpStepBack").click(() => {
			if (this.myVisualizer) {
				this.myVisualizer.stepBack();
			}
		});

		$("#jmpStepFwd").click(() => {
			if (this.myVisualizer) {
				this.myVisualizer.stepForward();
			}
		});
	...
	updateStepLabels() {
		...
		if (isLastInstr) {
			...
			$("#curInstr").html(
				`第 ${String(myVisualizer.curInstr)} 步，共 ${String(
					totalInstrs - 1
				)}步`
			);
		...
		if (
			isLastInstr &&
			myVisualizer.params.executeCodeWithRawInputFunc &&
			...
			ruiDiv
				...
				.click(() => {
					...
					if (TogetherJS.running) {
						alert(
							"Warning: user inputs do NOT work well in live help/chat mode. We suggest you use the regular Python Tutor visualizer instead."
						);
					}
		...
		if (
			...
			if (myVisualizer.curLineNumber) {
				...
				var markerId = s.addMarker(
					new Range(
						myVisualizer.curLineNumber - 1,
						0,
						myVisualizer.curLineNumber - 1,
						1
					),
			...
			$("#frontendErrorOutput").html(
				htmlspecialchars(myVisualizer.instrLimitReachedWarningMsg)
			);
		} else {
		...
		if (lineToScrollTo && !this.disableRowScrolling) {
			...
			if (lineToScrollTo < firstVisible || lineToScrollTo > lastVisible) {
				this.pyInputAceEditor.scrollToLine(
					lineToScrollTo,
					true /* try to center */
				);
			}
	...
	finishSuccessfulExecution() {
		...
		sliderDiv.slider({
			min: 0,
			max: myVisualizer.curTrace.length - 1,
			step: 1
		});
		...
		if (prevVisualizer) {
			...
			var prevVizIsFinalInstr =
				prevVisualizer.curInstr === prevVisualizer.curTrace.length - 1;
		...
		myVisualizer.updateHistory.push([
			myVisualizer.curInstr,
		...
		// above, perhaps using renderStep()
		myVisualizer.add_pytutor_hook("end_updateOutput", args => {
	...
	executeCodeAndCreateViz(
		...
		var execCallback = dataFromBackend => {
			...
			if (
				...
				this.myVisualizer = new ExecutionVisualizer(
					outputDiv,
					dataFromBackend,
					frontendOptionsObj
				);
		...
		if (jsonp_endpoint) {
			...
			$.get(
				...
				dat => {},
				"text"
			); // NOP handler since it's a dummy
			...
			if (pyState === "2" || pyState === "3") {
				$.get(
					backendScript,
					{
						user_script: codeToExec,
						raw_input_json:
							this.rawInputLst.length > 0
								? JSON.stringify(this.rawInputLst)
								: "",
						options_json: JSON.stringify(backendOptionsObj),
						user_uuid: this.userUUID,
						session_uuid: this.sessionUUID,
				...
				if (window.location.href.indexOf("localhost") >= 0) {
					...
					$.get(
						"http://localhost:3000/exec_js_native",
						{
							user_script: codeToExec,
							raw_input_json:
								this.rawInputLst.length > 0
